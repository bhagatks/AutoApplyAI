...
import { subscribeToJobs, saveJobToDb, deleteJobFromDb, saveUserProfile, getUserProfile, auth } from '../shared/db';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedHistory) {
      try {
        setJobs(JSON.parse(savedHistory));
      } catch (e) {}
    }
    ...
    if (auth) {
      const unsubAuth = onAuthStateChanged(auth, (user) => {
        ...
        if (user) {
          ...
          if (savedHist) {
            try {
              setJobs(JSON.parse(savedHist));
            } catch (e) {}
          }
          const savedProf = localStorage.getItem('candidateProfile');
          if (savedProf) {
            try {
              setCandidateProfile(JSON.parse(savedProf));
            } catch (e) {}
          }
        }
      });
          ...
          if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
            chrome.runtime.sendMessage(extId, {
              ...
              if (response && response.success) {
                ...
                console.error('Extension authentication failed:', response);

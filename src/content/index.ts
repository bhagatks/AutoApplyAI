// AutoApplyAI Bot - Content Script (TypeScript)
// Extracts job details and coordinates with the background script to run tailoring.

let isProcessing = false;

// Helper: Check if the extension context is still valid (or has been reloaded/invalidated)
...
const showWidget = (): HTMLDivElement => {
  injectCSS();
  let widget = document.getElementById('ag-apply-widget') as HTMLDivElement | null;
  if (!widget) {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    if (step) {
      ...
      const desc = step.querySelector('span:nth-child(2)') as HTMLSpanElement;
...
// Set state of a step
const setStepState = (stepNum: number, state: 'active' | 'success' | 'failed' | '', message: string | null = null) => {
  const step = document.getElementById(`ag-step-${stepNum}`);
  if (!step) return;

  step.className = `ag-step ${state}`;
  const bullet = step.querySelector('.ag-bullet') as HTMLSpanElement;
  const label = step.querySelector('span:nth-child(2)') as HTMLSpanElement;
...
const extractJobDescription = (): string => {
  const selectors = [
    ...
    '[data-automation-id="jobDescriptionText"]', // Workday Description
    '#content',
    '.section-wrapper',
    'main',
    'article'
...
const triggerAutomaticTailoring = async () => {
  ...
  // Request the background worker to execute the tailoring flow
  try {
...
// Listen for updates from the background worker and sidepanel requests
try {
...
// Helper: Check if the current page represents a job application / description page
const isJobPage = (): boolean => {
  const url = window.location.href.toLowerCase();
  
  // 1. Direct job boards/ATS patterns
  const jobDomains = [
    'linkedin.com/jobs',
    'indeed.com',
    'greenhouse.io',
  ...
  // 4. Search for Job Description headings (+2 points)
...
// Find the native apply button to inject next to it
const findNativeApplyButton = (): HTMLElement | null => {
  const commonSelectors = [
    '.jobs-s-apply', '.jobs-apply-button', // LinkedIn
    '.jobsearch-CallToActionButton', '#applyButtonLinkContainer', '.jobsearch-IndeedApplyButton', // Indeed
    '.page-apply-button', '.submit-application-btn', // Generic
    '#apply-button', '[data-automation-id="apply-button"]', // Workday
    '.postings-btn', '.apply-btn', // Lever / Greenhouse
    'a[href*="/apply"]', 'button[id*="apply"]', 'a[id*="apply"]' // General patterns
  ];
  ...
  // Fallback: Scan all buttons for text matching "apply"
  ...
  for (const btn of buttons) {
    ...
    if (
      ...
      text.includes('apply on') || 
      text.includes('apply to') || 
      text.includes('submit application')
    ) {
      if ((btn as HTMLElement).offsetWidth > 0 && (btn as HTMLElement).offsetHeight > 0) {
...
const injectButton = () => {
  ...
  if (!isContextValid()) {
    ...
    document.getElementById('autoapplyai-injected-btn')?.remove();
    document.getElementById('autoapplyai-floating-btn')?.remove();
    return;
  }

  // Verify we are on a job page
  if (!isJobPage()) {
    // If we navigated away, clean up any existing injected buttons
    document.getElementById('autoapplyai-injected-btn')?.remove();
    document.getElementById('autoapplyai-floating-btn')?.remove();
    return;
  }
  ...
  if (nativeApplyBtn) {
    // We found a native apply button! Inject inline and remove any floating tab
    document.getElementById('autoapplyai-floating-btn')?.remove();
    ...
    if (targetParent) {
      // Strict position check to prevent re-injection loops and blinking
      if (inlineBtn && inlineBtn.parentNode === targetParent && inlineBtn.nextSibling === nativeApplyBtn) {
        return;
      }
      
      // If it exists but in wrong place, remove first
      inlineBtn?.remove();
    ...
    // No native apply button found, but it is a job page! Inject the floating side tab button
    document.getElementById('autoapplyai-injected-btn')?.remove();
...
// Start MutationObserver to monitor dynamic DOM shifts and re-inject instantly
  ...
  observer = new MutationObserver((mutations) => {
    ...
    for (const mutation of mutations) {
      ...
      if (mutation.addedNodes.length > 0 || mutation.removedNodes.length > 0) {
        // Filter out mutations that only consist of our own injected buttons or style tags to prevent loops
...
checkInterval = setInterval(injectButton, 2000);
